...
public class StockScoreUpdate : MonoBehaviour
{
    //数字の画像
    ...
    {
        if (_currentDisplayScore == _stockScore) return;
        float localOldScore = _currentDisplayScore;
        if (_stockScore - _currentDisplayScore < 0) _currentDisplayScore += (float)_stockScore - _currentDisplayScore;
        else
        {
            //0.3秒でスコア加算の演出を終わらせるための変数  
            float _scoreMaxSec = 0.333f;
            //１ずつ加算していくようにするための加算方法
            localOldScore += (_stockScore - oldScore) / (_scoreMaxSec / Time.deltaTime);
