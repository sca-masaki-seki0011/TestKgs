...
public class DifficultyScoreUpdate : MonoBehaviour
{
    //数字の画像
    ...
    //現在のスコア
    private int _difficultyScore = 7000;
    ...
    {
        if (_currentDisplayScore == _difficultyScore) return;
        float localOldScore = _currentDisplayScore;
        if (_difficultyScore - _currentDisplayScore < 0) _currentDisplayScore += (float)_difficultyScore - _currentDisplayScore;
        else
        {
            //0.3秒でスコア加算の演出を終わらせるための変数  
            float _scoreMaxSec = 0.333f;
            //１ずつ加算していくようにするための加算方法
            localOldScore += (_difficultyScore - oldScore) / (_scoreMaxSec / Time.deltaTime);
